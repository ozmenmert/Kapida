...
$id = $_GET['id'] ?? -1;
$siparisDetay = $conn->query("SELECT * FROM a_siparisler WHERE siparis_id = '$id' ")->fetch(PDO::FETCH_ASSOC);
...
if (isset($_POST['odemeDurumGuncelle'])) {
    ...
    $update_query = $conn->prepare("UPDATE a_siparisler SET
            ...
            <div class="content-page-header">
                ...
                <div class="list-btn">
                    <ul class="filter-list">
                        ...
                        <li>
                            <a class="btn btn-primary" href="siparis-listesi"><i class="fa fa-list me-2"
                         ...
                         <table class="table table-striped">
                             ...
                             <tbody>
                                <?php foreach ($ozetSiparisler as $siparis) { 
                                      $urunBilgi = $conn->query("SELECT urun_adi FROM a_urunler WHERE urun_id = " . $siparis['siparis_urun'])->fetch(PDO::FETCH_ASSOC); ?>
                                  <tr>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-2">
                                    <div class='row'>
                                        <div class='radio'>
                                            ...
                                            while ($var = $veriCek->fetch(PDO::FETCH_ASSOC)) {
                                                $urunBilgi = $conn->query("SELECT * FROM a_urunler WHERE urun_id = " . $siparisDetay['siparis_urun'])->fetch(PDO::FETCH_ASSOC);
                                ...
                                <div class="col-md-6 mb-2">
                                    ...
                                    <div class='row'>
                                        ...
                                        $sql = "SELECT p.promosyon_id, p.promosyon_adi, p.promosyon_ucret FROM a_urunler_promosyonlar AS u_p
                                                LEFT OUTER JOIN a_promosyonlar AS p ON p.promosyon_id = u_p.promosyon_id
                                                WHERE u_p.urun_id = ".$siparisDetay['siparis_urun']."
                                                ORDER BY p.promosyon_ucret;";
                                        $promosyonSayi = $conn->query($sql)->fetchAll(PDO::FETCH_ASSOC);
                                        if (count($promosyonSayi)>0) { 
                                            
                                            //$veriCek = $conn->prepare("SELECT * FROM promosyon_urunler WHERE urun_id = :urun_id ORDER BY promosyon_ucret");
                                            //$veriCek->execute(array('urun_id' => $siparisDetay['siparis_urun']));
                                            //while ($var = $veriCek->fetch(PDO::FETCH_ASSOC)) {
                                            foreach ($promosyonSayi as $key=>$var) {
